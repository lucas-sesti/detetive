...
const SecretActionButton = ({ label, icon, onClick, danger }: { label: string, icon: React.ReactNode, onClick: () => void, danger?: boolean }) => (
  <MansionButton 
    onClick={onClick}
  ...
  return (
    ...
    >
      <MansionCard className="p-6 md:p-12 border-none bg-transparent">
        ...
        <div className="font-serif italic text-xl md:text-3xl leading-relaxed text-white/90 min-h-[200px] flex items-center justify-center px-2">
          <Typewriter 
...
const RandomEventPopup = ({ message }: { message: string }) => {
  return (
    ...
    >
      <MansionCard className="w-full max-w-md border-red-500/50 bg-red-950/20 text-center py-12 p-8 shadow-[0_0_50px_rgba(239,68,68,0.2)]">
        <motion.div
          animate={{ 
            rotate: [0, -1, 1, -1, 1, 0],
            scale: [1, 1.05, 1]
...
const InterrogationQuestionPopup = ({ question, timer }: { question: string, timer: number }) => {
  return (
    ...
    >
      <MansionCard className="w-full max-w-lg border-white/20 bg-black/40 p-8 md:p-12 text-center shadow-[0_0_100px_rgba(255,255,255,0.1)]">
        <MansionLabel className="mb-6 tracking-[0.5em] text-white/40 block">BENOÎT BLANC PERGUNTA:</MansionLabel>
         ...
         <div className="mt-8 flex flex-col items-center gap-4">
           ...
           <p className="text-xs text-white/40 uppercase tracking-widest font-bold">Defesa inicia em {timer}s...</p>
         </div>
      </MansionCard>
    </motion.div>
  );
...
const TransitionView = ({ timer }: { timer: number }) => {
  return (
    ...
    >
      ...
      >
        <MansionLabel className="mb-4 tracking-[0.6em] md:tracking-[1em] text-white/40 block">MOMENTO DO CRIME</MansionLabel>
...
const NotificationPopup = ({ notification }: { notification: { message: string, type: string } }) => {
  ...
  return (
    ...
    >
      <MansionCard className="w-full max-w-sm text-center py-12 p-8 border-white/20 shadow-[0_0_100px_rgba(255,255,255,0.1)]">
        <motion.div
        ...
        <MansionLabel className="mb-4 text-xs font-black tracking-[0.4em] text-white/40">ALERTA DE SEGURANÇA</MansionLabel>
...
export default function App() {
  ...
  const [actionTargeting, setActionTargeting] = useState<{ type: SecretActionType, firstTargetId?: string } | null>(null);
  ...
  if (!gameState) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <MansionCard className="w-full max-w-md p-6 sm:p-8">
          <h1 className="text-2xl sm:text-4xl font-serif text-center mb-8 tracking-tighter">DEAD MAN'S PARTY</h1>
          <div className="space-y-6">
            ...
            <div>
              <MansionLabel>Escolha seu Avatar</MansionLabel>
                ...
                {AVATARS.map(a => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isTaken}
                      className={cn(
                        "text-3xl p-3 border rounded-xl transition-all flex items-center justify-center",
                        selectedAvatar === a ? "border-white bg-white/10" : "border-white/10 hover:bg-white/5",
                        isTaken && "opacity-20 cursor-not-allowed grayscale"
                      )}
                    >
                      <AvatarDisplay avatar={a} className="w-10 h-10 flex items-center justify-center" />
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-3 pt-4">
              <MansionButton onClick={handleCreateRoom} className="w-full">
                Criar Nova Sala
              </MansionButton>
  ...
  if (gameState.phase === GamePhase.LOBBY) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <MansionCard className="w-full max-w-lg">
          <div className="flex justify-between items-center mb-6">
            <div>
              <MansionLabel>Sala</MansionLabel>
              <h2 className="text-2xl font-mono tracking-widest">{gameState.roomId}</h2>
            </div>
            <MansionBadge>{gameState.players.length}/8 Jogadores</MansionBadge>
          </div>

          <div className="space-y-3 mb-8">
            <MansionLabel>Jogadores</MansionLabel>
            <div className="grid grid-cols-2 gap-2">
              {gameState.players.map(p => (
                <div key={p.id} className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/5 shadow-sm">
                  <AvatarDisplay avatar={p.avatar} className="w-8 h-8 flex items-center justify-center text-xl shrink-0" />
                  <span className="text-sm font-medium">{p.nickname}</span>
                  {p.isHost && <span className="text-[8px] uppercase bg-white/10 px-1 rounded ml-auto">Host</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          {me?.isHost && (
            <div className="space-y-3">
              ...
              >
                {gameState.players.length < 4 ? 'Aguardando mais jogadores (Mín. 4)' : 'Iniciar Mistério'}
              </MansionButton>
              <MansionButton 
          ...
          {!me?.isHost && (
            <p className="text-center text-white/50 text-xs italic">Aguardando o Host iniciar...</p>
          )}
        </MansionCard>
      </div>
    );
  ...
  const ItemIcon = ({ item }: { item?: Item }) => {
    switch (item) {
      case Item.KNIFE: return <Sword className="w-5 h-5 text-red-400" />;
      case Item.GLASSES: return <Glasses className="w-5 h-5 text-blue-400" />;
      case Item.DRINK: return <Wine className="w-5 h-5 text-blue-400" />;
      case Item.WATCH: return <Watch className="w-5 h-5 text-blue-400" />;
      case Item.RING: return <Circle className="w-5 h-5 text-blue-400" />;
      case Item.LETTER: return <Mail className="w-5 h-5 text-blue-400" />;
      case Item.PHOTOGRAPH: return <Image className="w-5 h-5 text-blue-400" />;
      case Item.KEY: return <Key className="w-5 h-5 text-blue-400" />;
      case Item.POCKET_WATCH: return <Clock className="w-5 h-5 text-blue-400" />;
      case Item.CIGARETTE_CASE: return <Briefcase className="w-5 h-5 text-blue-400" />;
      default: return <HandMetal className="w-5 h-5 text-blue-400" />;
    }
  };
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center">
            ...
            <MansionCard className="text-center py-12">
              {!me?.roleRevealed ? (
                <div className="space-y-6">
                  ...
                  <h1 className="text-2xl sm:text-4xl font-serif mb-8">Investigue sua identidade...</h1>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="mb-6 p-4 bg-red-950/20 border border-red-500/20 rounded-2xl">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {killers.filter(k => k.id !== socket.id).map(k => (
                          <span key={k.id} className="text-lg font-bold">{k.avatar} {k.nickname}</span>
                        ))}
                      </div>
                    </div>
                  )}

                  <div className="bg-white/5 py-8 px-4 rounded-3xl border border-white/10 inline-block mb-6 min-w-[200px]">
                    <MansionLabel>Seus Itens</MansionLabel>
                    <div className="flex flex-col items-center gap-6 mt-4">
                      {me?.hasKnife && (
                        <div className="flex flex-col items-center gap-2">
                           <ItemIcon item={Item.KNIFE} />
                           <span className="text-2xl font-serif text-red-100">{Item.KNIFE}</span>
                           <span className="text-[10px] uppercase font-bold text-red-500/50 -mt-2">Arma do Crime</span>
                        </div>
                      )}
                      <div className="flex flex-col items-center gap-2">
                        ...
                        {me?.hasKnife && <span className="text-[10px] uppercase font-bold text-white/20 -mt-2">Disfarce</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  {me?.logs && me.logs.length > 0 && (
                    <div className="mb-6 text-left max-w-xs mx-auto space-y-2">
                       <MansionLabel className="text-[10px] opacity-40">Informações Coletadas</MansionLabel>
              ...
              >
                <MansionCard className="p-8 border-l-4 border-l-white relative bg-black/40">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <MansionBadge className="bg-white/5 border-white/10 text-white/40 uppercase tracking-tighter text-[9px] font-bold">
                      ...
                      {gameState.players.filter(p => p.isAlive).map(p => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {me?.gossipVote === p.id && (
                              <div className="w-6 h-6 rounded-full bg-black flex items-center justify-center text-[10px]">
                                {me.avatar}
                              </div>
                            )}
                            ...
                            <div className="flex -space-x-1">
                              {gameState.players.filter(v => v.hasGossipVoted && v.gossipVote === p.id && v.id !== socket.id).map(v => (
                                <div key={v.id} className="w-5 h-5 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-[8px] animate-pulse">
                                   ?
                                </div>
                              ))}
                            </div>
              ...
              >
                <MansionCard className={cn(
                  ...
                  <div className="absolute top-4 left-0 w-full px-4 flex justify-between items-center">
                    <MansionBadge className="bg-red-500 text-white border-none animate-pulse">Momento do Crime</MansionBadge>
                    <span className="text-xs font-mono text-red-500 font-bold">{gameState.timer}s</span>
                  </div>
                      ...
                      {actionTargeting ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                             {gameState.players.filter(p => p.isAlive && p.id !== socket.id && p.id !== actionTargeting.firstTargetId).map(p => (
                               ...
                               >
                                  <AvatarDisplay avatar={p.avatar} className="w-6 h-6 flex items-center justify-center text-xl shrink-0" />
                                  <span className="text-xs font-bold truncate">{p.nickname}</span>
                               </button>
                             ))}
                          </div>
                          <MansionButton variant="secondary" className="w-full text-[10px]" onClick={() => setActionTargeting(null)}>
                             CANCELAR
                          </MansionButton>
                        </motion.div>
                      ) : (
                        <div className="flex flex-col items-center gap-6">
                           <motion.div
                            ...
                            transition={{ repeat: Infinity, duration: 2 }}
                          >
                             <MansionLabel className="mb-2 block italic text-red-400 font-bold">
                                SUA VEZ DE AGIR:
                             </MansionLabel>
                          </motion.div>
                          ...
                          <div className="w-full flex justify-center">
                             <SecretActionButton 
                              label={me.assignedSecretAction} 
                               icon={
                                 me.assignedSecretAction === SecretActionType.SNOOP ? <Eye className="w-8 h-8"/> :
                                 me.assignedSecretAction === SecretActionType.STEAL ? <HandMetal className="w-8 h-8"/> :
                                 me.assignedSecretAction === SecretActionType.SWAP ? <Shuffle className="w-8 h-8"/> :
                                 me.assignedSecretAction === SecretActionType.SHUFFLE ? <Users className="w-8 h-8"/> :
                                 me.assignedSecretAction === SecretActionType.ALIBI ? <ShieldCheck className="w-8 h-8"/> :
                  ...
                  {snoopResult && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 p-4 bg-white/10 rounded-2xl border border-white/20 w-full">
                      <MansionLabel>Espionagem</MansionLabel>
                      <p className="text-sm">
                        {gameState.players.find(p => p.id === snoopResult.targetId)?.nickname} tem o item {snoopResult.item}
                      </p>
                    </motion.div>
                  )}
                </MansionCard>
          ...
          <motion.div key="interrogation" className="w-full max-w-4xl py-12 pb-40">
             <AnimatePresence>
               {gameState.isInterrogationQuestionWindow && gameState.interrogationQuestion && (
                 <InterrogationQuestionPopup question={gameState.interrogationQuestion} timer={gameState.timer} />
               )}
             </AnimatePresence>
                  ...
                  >
                    <MansionCard className={cn(
                      ...
                      {(!gameState.isInterrogationQuestionWindow) && gameState.interrogationQuestion && (
                        ...
                        >
                          <MansionLabel className="text-white/60 mb-2 block">PERGUNTA PARA O SUSPEITO</MansionLabel>
                          <p className="text-xl font-serif italic text-white font-bold">"{gameState.interrogationQuestion}"</p>
                        </motion.div>
                      )}

                      <MansionLabel className={gameState.gossipResults[gameState.interrogationIndex].mostVotedId === socket.id ? "text-red-400" : ""}>
                        A Acusação Popular
                      </MansionLabel>
                      ...
                      <div className={cn(
                        ...
                        <p className="text-[10px] uppercase font-black tracking-[0.4em] mb-3 opacity-60">O Principal Suspeito</p>
                        <div className="flex items-center justify-center gap-3">
                           <AvatarDisplay avatar={gameState.players.find(p => p.id === gameState.gossipResults[gameState.interrogationIndex].mostVotedId)?.avatar || '🤵'} className="w-10 h-10 flex items-center justify-center text-4xl" />
                           <p className={cn(
                            "font-serif text-3xl",
                      ...
                      <div className="mt-12 flex flex-col items-center gap-4">
                        ...
                        <p className="text-xs text-white/40 uppercase tracking-widest font-bold">
                          {gameState.timer > 40 ? `Iniciando em ${gameState.timer - 40}s...` : `Defenda-se: ${gameState.timer}s`}
                        </p>
                         ...
                         {gameState.timer <= 40 && (
                          <div className="w-full mt-8 max-w-xs">
                             ...
                             >
                               ...
                               {me?.isReadyToSkip && <span className="ml-2 inline-block w-4 h-4 rounded-full border-2 border-white/20 border-t-white animate-spin" />}
                             </MansionButton>
                          </div>
                        )}
                      </div>
             ...
             <MansionCard className="p-8 text-center bg-black/20 border-white/5 backdrop-blur-sm max-w-2xl mx-auto">
                ...
                <p className="text-xs text-white/40 italic leading-relaxed">
                  Benoît Blanc exige saber: Você realizou ações? Viu algo suspeito? Alguém roubou seu item? <br/>
                ...
                {gameState.players.filter(p => p.isAlive).map(p => (
                   <div key={p.id} className={cn(
                     ...
                     <div className="flex items-center gap-4">
                        <AvatarDisplay avatar={p.avatar} className="w-12 h-12 flex items-center justify-center text-3xl shrink-0" />
                        <span className="font-medium">{p.nickname}</span>
                     </div>
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <div className="flex -space-x-2">
                          {gameState.players.filter(v => v.votedFor === p.id).map(v => (
                            <div key={v.id} className="w-6 h-6 rounded-full border border-black bg-black/20 flex items-center justify-center text-[10px]">
                              {v.avatar}
                            </div>
                          ))}
                        </div>
          ...
          <motion.div key="gameover" className="w-full max-w-md text-center">
            <MansionCard className="py-12 border-2 border-white/30">
               ...
               <div className="space-y-4 mb-8">
                  ...
                  <div className="space-y-2">
                    {gameState.players.map(p => (
                      <div key={p.id} className="flex items-center justify-between px-4 py-2 bg-white/5 rounded-xl text-sm">
                        <div className="flex items-center gap-2">
                          <AvatarDisplay avatar={p.avatar} className="w-5 h-5 flex items-center justify-center shrink-0" />
                          <span className={cn(!p.isAlive && "line-through opacity-50")}>{p.nickname}</span>
                        </div>
                        <span className={cn(
      ...
      {/* Item Display footer */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-sm px-4 flex flex-col gap-2 pointer-events-none">
          {me && gameState.phase !== GamePhase.LOBBY && gameState.phase !== GamePhase.TRANSITION && gameState.phase !== GamePhase.INTRO && gameState.phase !== GamePhase.GAME_OVER && (
          <>
            {me.logs && me.logs.length > 0 && (
              <motion.div 
                initial={{ opacity: 0, y: 10 }} 
              ...
              >
                <MansionLabel className="text-[9px] mb-1 opacity-50">Inteligência Coletada</MansionLabel>
                <div className="space-y-1">
                  {me.logs.map((log, i) => (
            ...
            <MansionCard className="py-3 px-4 flex items-center justify-between border-white/20 bg-black/95 backdrop-blur-xl pointer-events-auto shadow-2xl">
               <div className="flex items-center gap-3">
                 <AvatarDisplay avatar={me.avatar} className="w-10 h-10 flex items-center justify-center text-2xl" />
                         ...
                         <>
                           {me.itemExposed ? (
                             <div className="flex items-center gap-1">
                               {me.hasKnife && <span className="text-red-500 font-bold tracking-tighter">[{Item.KNIFE}]</span>}
